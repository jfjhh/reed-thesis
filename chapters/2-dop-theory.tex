...
outcome with some probability. An experiment constitutes a \term{measurement} if
...
able to undo a measurement by forgetting the outcome.\todo[inline]{Clarify and
distinguish selective vs nonselective measurements, and relate to decoherence.}
...
A quantum state $\dop$ of \term{dimension} $N$ represents one's beliefs about
...
\begin{post}\label{post:dop}
  A \term{state} is described by a \term{density operator} $\dop$,
...
\begin{post}
  An \term{operation}\footnote{Quantum operations are also called generalized
...
formal linear combinations. The classes are known as \term{tensors}, and the
corresponding map $\tp$ from $\liou_1 \times \liou_2$ to the space of tensors
$\liou_1 \tp \liou_2$ is known as the \term{tensor product}. The composite of
...
\begin{post}
  Operation maps are \term{completely positive}. That is, for any Liouville
...
generalizes \cref{eq:liouville} to typically non-unitary \textsc{cptp} maps of
...
We have seen how the general consideration of Markovian \textsc{cptp} maps on
