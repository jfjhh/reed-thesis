...
is\footnote{The time dependence of the field is absorbed into
$\opr{a}_{\vq{k},\lambda}$.}
\begin{align}
  \ham_I
...
\end{equation}
\todo[inline]{Explicit calculation.}
\begin{align}
...
\end{align}
the spectral correlation tensor becomes
\begin{align}
  \begin{split}
  \Gamma_{i\mu,j\nu}(\omega)
...
\todo[inline]{%
  ...
  optical phonon branch. Magnons? Just no absorption for off frequencies?
}

To evaluate \cref{eq:Gamma-precontinuum}, we now consider a chain of $N$ spins
along the $z$-axis, so that $\vq{r}_i = ia \vu{z}$.\footnote{We could consider
any axis given the spherical symmetry, but the $z$-axis is the simplest to
evaluate.} Then $\vq{k} \vdot (\vq{r}_i - \vq{r}_j) = (i - j)k^z a$.
...
\begin{equation}
  ...
  = 4\pi k^2 \delta_{\mu\nu} \sinc(ka(i - j)).
\end{equation}
We are now left with (TODO)
\begin{align}
  \begin{split}
  \Gamma_{i\mu,j\nu}(\omega)
  &= \frac{c^2 \mu_0}{2\hbar} \int_0^\infty \dd{s}
  \frac{1}{{(2\pi c)}^3} \int_0^\infty \dd{\omega_k} \omega_k^2
  \int\dd{\Omega}
  \frac{1}{\omega_k}
  :
  ...
  &{\qty(\vq{k} \cross \vq{e}_{\vq{k},\lambda})}_\mu
    {\qty(\vq{k} \cross \vq{e}^*_{\vq{k},\lambda})}_\nu
    e^{\im(\vq{k}\vdot(\vq{r}_i - \vq{r}_j) + s(\omega - \omega_k))}
    \qty(1 + n_B(\omega_k))
  \\
  &+
    {\qty(\vq{k} \cross \vq{e}^*_{\vq{k},\lambda})}_\mu
...
\end{split} \\
\begin{split}
  &= \delta_{\mu\nu} \frac{c^2 \mu_0}{2\hbar} \int_0^\infty \dd{s}
  \frac{1}{{(2\pi c)}^3} \int_0^\infty \dd{\omega_k} \omega_k^2
  \frac{1}{\omega_k}
  \frac{4\pi \omega_k^2}{c^2} \sinc(\omega_k a(i - j)/c) \\
  &\cdot
  \qty(e^{\im s(\omega - \omega_k)} \qty(1 + n_B(\omega_k))
  + e^{\im s(\omega + \omega_k)} n_B(\omega_k))
\end{split} \\
\begin{split}
  &= \delta_{\mu\nu}
  \frac{\mu_0}{4\hbar\pi^2 c^3}
  \int_0^\infty \dd{\omega_k}
  \omega_k^3 \sinc(\omega_k a(i - j)/c) \\
  &\cdot
  \int_0^\infty \dd{s}
  \qty(e^{\im s(\omega - \omega_k)} \qty(1 + n_B(\omega_k))
  + e^{\im s(\omega + \omega_k)} n_B(\omega_k))
...
\begin{equation}
  \int\dd{\Omega} \sum_\lambda {\qty(\vq{k} \cross \vq{e}_{\vq{k},\lambda})}_\mu
  {\qty(\vq{k} \cross \vq{e}^*_{\vq{k},\lambda})}_\nu
  = 4\pi k^2 \delta_{\mu\nu} - \int\dd{\Omega} k^\mu k^\nu.
\end{equation}
By symmetry, the integrand must have $\mu = \nu$ to not vanish. In that case,
the integral gives the volume of the sphere with radius $k$. Now the integral
reduces to
\begin{equation}
  \int\dd{\Omega} \sum_\lambda {\qty(\vq{k} \cross \vq{e}_{\vq{k},\lambda})}_\mu
  {\qty(\vq{k} \cross \vq{e}^*_{\vq{k},\lambda})}_\nu
...
\end{equation}
Substituting this into \cref{eq:Gamma-precontinuum} gives the continuum limit:
(TODO)
\begin{align}
  \begin{split}
  \Gamma_{i\mu,j\nu}(\omega)
  &= \delta_{\mu\nu} \frac{\mu_0}{6\hbar\pi^2 c^3}
   \int_0^\infty \dd{\omega_k} \omega_k^3
  :
  \\
  & \qty(1 + n_B(\omega_{\vq{k}})) e^{\im \vq{k}\vdot(\vq{r}_i - \vq{r}_j)}
  \int_0^\infty \dd{s} e^{\im s(\omega - \omega_{\vq{k}})}
  \\
  &+ n_B(\omega_{\vq{k}}) e^{-\im\vq{k}\vdot(\vq{r}_i - \vq{r}_j)}
  \int_0^\infty \dd{s} e^{\im s(\omega + \omega_{\vq{k}})}.
\end{split}
\end{align}
We now use that
...
\end{equation}
where $\pv$ denotes the Cauchy principal value, to find (TODO)
\begin{align}
  \begin{split}
  \Gamma_{i\mu,j\nu}(\omega)
  &= \delta_{\mu\nu} \frac{\mu_0}{6\hbar\pi^2 c^3}
