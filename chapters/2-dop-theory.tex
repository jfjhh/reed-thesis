...
\dropcap{I}{f quantum mechanics} is so weird, then why aren't we? The answer lies in the
phenomenon of \term{decoherence}, which was first considered in depth in the
...
The Hilbert space of the composite system is the \term{tensor product}
$\hilb = \hilb_A \tp \hilb_B$ (\cref{def:tensors}). How might a density operator $\dop$ for the
composite system admit a \term{reduced density operator} $\dop_A$ for system $A$?
Consider an observable $\opr{O}$ of system $A$, for which the corresponding
composite observable is $\opr{O} \tp \idopr$. Regardless of the
...
\begin{defn}[Tensor product]\label{def:tensors}
  ...
  is a unique bilinear map $g:V \tp W \to Z$ such that $h = g \circ \phi$.
  ...
  correspondence between the element $v \tp f$ of the tensor product $V
  \tp V^*$ and the linear map $T:V \to V$ defined by $Tx = f(x)v$.
