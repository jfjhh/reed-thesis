...
neglected just like we neglect the zero-point energy of the bath. Thus the
unitary part of the reduced dynamics exactly matches that of the closed system,
and $\sopr{L} = \sopr{D}$ in the interaction picture. The dipole approximation
then leaves
\begin{equation}
  \gamma(\omega)
...
\end{equation}
for all spins and polarizations. Nondimensionalizing to $\tilde{\gamma} = \gamma
\tau_S$ gives
\begin{equation}
...
\begin{equation}
  ...
  \cong \SI{56}{\per\GeV}.
\end{equation}
If $\tau_B/\tau_S \ll \tilde{\omega}$, then
\begin{equation}
  \tilde{\gamma}(\tilde{\omega})
