...
\end{align}
\todo[inline]{%
  Should the $\opr{\eta}_k$ match Pfeuty? If so, then hmm: these coefficients
  are complex while Pfeuty's are real. But I don't think they need to match.
  Having the same Hamiltonian and spectrum of energies says that the two
  representations of the Hamiltonian in the occupation basis for the different
  $\opr{\eta}_k$ are the same (up to permutation). However, since the spectrum
  is degenerate (Mathematica), these states may be rotated around within each
  degenerate eigenspace without changing the representation. It is thus possible
  that Pfeuty's and my $\opr{\eta}_k$ are both correct. While my way of
  obtaining the $\opr{\eta}_k$ through a Fourier transform is more
  straightforward, it gives more complicated coefficients for the $\opr{c}_i$
  than the method of Lieb et.\ al., which is less direct (a harder eigenvalue
  problem than $2 \times 2$ matrices), but gives nice coefficients for the
  $\opr{c}_i$~\cite[pp.~452--454]{liebTwoSolubleModels1961}.
}
We recall \cref{eq:paulix} as
\begin{align}
  \pauli_i^x
